...
import type { Pill, TokenPill, ChoicePill, InputPill, WrapperPill, ComplexPill } from "./types";

// The unit tests below intentionally probe makePill/pillTokens with minimal pill shapes;
// `asPill` documents that these are deliberately-partial fixtures for behaviour checks.
const asPill = (o: Record<string, unknown>): Pill => o as unknown as Pill;
...
describe("makePill", () => {
  it("builds a toggle pill carrying its token", () => {
    const p = makePill(CAT_BY_ID.p_hdr) as TokenPill;
  ...
  it("builds a tool pill", () => {
    expect((makePill(CAT_BY_ID.t_mangohud) as TokenPill).token).toBe("mangohud");
  });
  it("builds a choice pill defaulting to its default value", () => {
    const p = makePill(CAT_BY_ID.d_hud) as ChoicePill;
...
describe("pillTokens", () => {
  it("toggle/tool/custom emit their token (or nothing when empty)", () => {
    expect(pillTokens(asPill({ kind: "toggle", token: "A=1" }))).toEqual(["A=1"]);
    expect(pillTokens(asPill({ kind: "tool", token: "mangohud" }))).toEqual(["mangohud"]);
    expect(pillTokens(asPill({ kind: "custom", token: "" }))).toEqual([]);
  });
  it("choice/input emit KEY=VALUE", () => {
    expect(pillTokens(asPill({ kind: "choice", key: "DXVK_HUD", value: "fps" }))).toEqual(["DXVK_HUD=fps"]);
    expect(pillTokens(asPill({ kind: "input", key: "LD_PRELOAD", value: "" }))).toEqual(["LD_PRELOAD="]);
  });
  it("wrapper emits its head only when present", () => {
    expect(pillTokens(asPill({ kind: "wrapper", head: "game" }))).toEqual(["game"]);
    expect(pillTokens(asPill({ kind: "wrapper", head: "" }))).toEqual([]);
  });
  it("complex delegates to gamescopeTokens", () => {
    const toks = pillTokens(asPill({ kind: "complex", cfg: GAMESCOPE_DEFAULT }));
    expect(toks).toContain("gamescope");
    expect(toks[toks.length - 1]).toBe("--");
  });
  it("unknown kind emits nothing", () => {
    expect(pillTokens(asPill({ kind: "nope" }))).toEqual([]);
  });
});
...
describe("parseLine", () => {
  ...
  it("parses env toggles + a known wrapper", () => {
    ...
    expect((pills[1] as WrapperPill).head).toBe("game");
  });

  it("parses choice and input env tokens to their pills", () => {
    const pills = parseLine("DXVK_HUD=full DXVK_FRAME_RATE=120 game %command%");
    const choice = pills.find((p) => p.kind === "choice") as ChoicePill;
    const input = pills.find((p) => p.kind === "input") as InputPill;
  ...
  it("parses a tool prefix", () => {
    ...
    expect((pills.find((p) => p.kind === "tool") as TokenPill).token).toBe("mangohud");
  });
  ...
  it("treats an unknown wrapper as a raw wrapper pill carrying the literal head", () => {
    ...
    const w = pills.find((p) => p.kind === "wrapper") as WrapperPill;
  ...
  it("parses a full gamescope line back into a complex pill with cfg", () => {
    ...
    const gs = pills.find((p) => p.kind === "complex") as ComplexPill;
    ...
    expect(pills.some((p) => p.kind === "toggle" && (p as TokenPill).token === "PROTON_USE_NTSYNC=1")).toBe(true);
  });
});
